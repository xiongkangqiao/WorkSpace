  ...
  export default {
    ...
    data() {
      ...
      img_logo: require('@/assets/img/logo.png'),
        ...
        menuChild: [
          {name: '学术会议',newsType:0},
          {name: '国内交流',newsType:1},
          {name: '国际合作',newsType:2},
        ],
        newsType:0,
      }
    },
    methods: {
      ...
      clickNews(row){
        this.$router.push({path:'/zxdt/xq',query: {id: row.id, subjectType: '2'}});
        // this.$router.push({path:'/home',query: {id:'1'}})
      },
      getData(n1) {
        // 这里使用axios，使用时请提前引入
        this.axios.post(this.$store.state.interface+'/web/cooperation/list',{
